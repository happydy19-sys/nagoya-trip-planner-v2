...
import { 
  ...
  ArrowUp, ArrowDown, ExternalLink, Moon, Sun,
  CheckSquare, Info, Camera, Train, Sparkles, 
  Footprints, Bus, Banknote, Navigation
} from 'lucide-react';
...
type EventType = 'transport' | 'food' | 'sight' | 'shopping' | 'relax';
...
interface Expense {
  ...
  currency: 'JPY' | 'TWD';
  date: string;
  rateAtTimeOfEntry: number; // 鎖定的匯率
}

interface NewExpenseForm {
  id: number;
  item: string;
  cost: string; // 保持 string 類型以便與 input 綁定
  currency: 'JPY' | 'TWD';
  date: string;
}
...
const INITIAL_TRIP_DATA: TripDay[] = [
  // Day 1
  {
    ...
    weather: { temp: '4°C - 12°C', icon: 'sun', desc: '晴時多雲', wear: '建議洋蔥式穿搭，犬山城風大需圍巾。' },
  ...
  // Day 2
  {
    ...
    weather: { temp: '3°C - 11°C', icon: 'cloud', desc: '多雲陰天', wear: '園區很大都在走路，請穿最好走的鞋。' },
  ...
  // Day 3
  {
    ...
    weather: { temp: '5°C - 13°C', icon: 'sun', desc: '晴朗乾燥', wear: '海邊晚上看煙火極冷，帽子手套暖暖包必備。' },
  ...
  // Day 4
  {
    ...
    weather: { temp: '-2°C - 5°C', icon: 'snow', desc: '山區降雪', wear: '山區極冷！發熱衣、防滑鞋、毛帽、手套。' },
  ...
  // Day 5
  {
    ...
    weather: { temp: '6°C - 14°C', icon: 'sun', desc: '晴時多雲', wear: '輕鬆便裝，方便在商場試穿衣服。' },
...
export default function App() {
  ...
  const deleteEvent = (dayId: number, eventId: string) => {
  ...
  const addExpense = () => {
    ...
    const rate = exchangeRate || 0;
    const costValue = parseFloat(newExpense.cost); // 轉換為數字

    if (isNaN(costValue)) return; // 安全檢查
    ...
    setExpenses([...expenses, { 
      ...
      // 確保 currency 是正確的類型
      currency: newExpense.currency as 'JPY' | 'TWD' 
    }]);
  ...
  const getEventStyle = (type: EventType) => {
    switch(type) {
      case 'food': return { icon: <Utensils size={18} />, color: 'text-orange-600', bg: 'bg-orange-50', border: 'border-orange-200' };
      case 'sight': return { icon: <Camera size={18} />, color: 'text-red-700', bg: 'bg-white', border: 'border-red-100' };
      case 'shopping': return { icon: <ShoppingBag size={18} />, color: 'text-pink-600', bg: 'bg-pink-50', border: 'border-pink-200' };
      case 'relax': return { icon: <Moon size={18} />, color: 'text-indigo-600', bg: 'bg-indigo-50', border: 'border-indigo-200' };
            ...
            {tripData.filter(d => d.id === currentDayId).map((day: TripDay) => (
              <div key={day.id} className="animate-fadeIn px-4 pt-4">
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-2">
                      <div className="bg-blue-50 p-2 rounded-lg text-blue-600">
                        {day.weather.icon === 'sun' ? <Sun size={20} /> : <CloudSun size={20} />}
                      </div>
                      <div>
                ...
                <div className="relative pb-4">
                  {day.events.map((event: TripEvent, idx: number) => {
                        ...
                        <div key={event.id} className="relative flex pl-2 pb-8">
                          ...
                          <div className="relative z-10 w-8 flex flex-col items-center pt-1">
                             <div className="bg-stone-200 text-stone-500 p-1.5 rounded-full ring-4 ring-stone-50">
                               {event.title.includes('飛機') ? <Plane size={14}/> : 
                                event.title.includes('巴士') ? <Bus size={14} /> :
                                event.title.includes('走') ? <Footprints size={14} /> :
                                <Train size={14} />}
                             </div>
                          </div>
                          ...
                          <div className="flex-1 ml-4 pt-1">
                            ...
                            {isEditMode && (
                              <div className="flex gap-2 mt-2">
                                <button onClick={() => moveEvent(day.id, idx, 'up')} className="p-1 bg-stone-200 rounded text-stone-600"><ArrowUp size={12} /></button>
                                <button onClick={() => moveEvent(day.id, idx, 'down')} className="p-1 bg-stone-200 rounded text-stone-600"><ArrowDown size={12} /></button>
                                <button onClick={() => deleteEvent(day.id, event.id)} className="p-1 bg-red-100 rounded text-red-600"><Trash2 size={12} /></button>
                              </div>
                            )}
                          </div>
                        </div>
...
                                >
                          ...
                          {event.guide && (
                           ...
                           {isEditMode && (
                              <div className="flex gap-2 mt-3 pt-2 border-t border-stone-100">
                                <button onClick={() => moveEvent(day.id, idx, 'up')} className="p-1 bg-stone-100 rounded text-stone-600"><ArrowUp size={14} /></button>
                                <button onClick={() => moveEvent(day.id, idx, 'down')} className="p-1 bg-stone-100 rounded text-stone-600"><ArrowDown size={14} /></button>
                                <button onClick={() => deleteEvent(day.id, event.id)} className="p-1 bg-red-50 rounded text-red-600"><Trash2 size={14} /></button>
                              </div>
                           )}
                        </div>
                         {/* Time line connector */}
        ...
        {activeTab === 'wallet' && (
          <div className="p-4 space-y-6">
            ...
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-stone-200">
              ...
              <div className="bg-red-50 rounded-lg p-3 flex items-center justify-between gap-2 text-sm border border-red-100 mb-3">
                ...
                <input 
                  ...
                  value={exchangeRate}
                  onChange={(e) => setExchangeRate(parseFloat(e.target.value) || 0)} // 確保儲存為 number
                ...
                <div className="flex gap-2">
                  <div className="relative w-24 shrink-0">
                    <select 
                      ...
                      onChange={e => setNewExpense({...newExpense, currency: e.target.value as 'JPY' | 'TWD'})}
                    >
            ...
            {/* 紀錄列表 */}
            <div className="space-y-3">
              <h3 className="text-sm font-bold text-stone-500 pl-1">消費紀錄</h3>
              {expenses.length === 0 && (
              ...
              {expenses.slice().reverse().map((ex: Expense) => { 
                ...
                return (
                  <div key={ex.id} className="bg-white p-4 rounded-xl border border-stone-100 shadow-sm">
                    <div className="flex justify-between items-center mb-1">
                      {/* 品項 */}
                      <span className="text-stone-700 font-medium max-w-[60%] truncate">{ex.item}</span>
                    ...
                    <div className="flex justify-between items-end text-xs text-stone-400 border-t border-stone-50 pt-1">
                        ...
                        {isJPY && <span className="text-[10px] text-stone-400">鎖定匯率: {rateToUse.toFixed(4)}</span>}
                    </div>
          ...
          <div className="p-4 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden">
               <div className="bg-indigo-900 p-4 text-white flex justify-between items-center">
                 <div className="font-bold flex items-center gap-2"><Plane size={18}/> 航班資訊</div>
                 <div className="text-xs bg-indigo-800 px-2 py-1 rounded text-indigo-200">Peach 樂桃</div>
               </div>
               ...
               <div className="p-5 space-y-6">
                 ...
                 <div className="relative">
                    <div className="flex justify-between items-end mb-2">
                      <span className="text-3xl font-black text-stone-800">TPE</span>
                      <span className="text-xs font-mono text-stone-400 mb-1 flex-1 text-center">MM722 • 2h 30m</span>
                      <span className="text-3xl font-black text-stone-800">NGO</span>
                    </div>
                 ...
                 <div className="relative pt-4 border-t border-stone-100">
                    <div className="flex justify-between items-end mb-2">
                      <span className="text-3xl font-black text-stone-800">NGO</span>
                      <span className="text-xs font-mono text-stone-400 mb-1 flex-1 text-center">MM723 • 3h 40m</span>
                      <span className="text-3xl font-black text-stone-800">TPE</span>
                    </div>
                    <div className="w-full h-1 bg-stone-100 rounded-full overflow-hidden flex">
                      <div className="w-1/2 bg-indigo-500 h-full"></div>
            ...
            <div className="bg-red-50 rounded-2xl p-5 border border-red-100">
              ...
              <div className="grid grid-cols-2 gap-3">
                {EMERGENCY_CONTACTS.map((c, i) => (
                  <a key={i} href={`tel:${c.phone}`} className="bg-white p-3 rounded-xl shadow-sm border border-red-100 active:bg-red-50 transition-colors">
                    <div className="text-xs text-stone-400 mb-1">{c.name}</div>
                    <div className="text-base font-bold text-red-700 font-mono">{c.phone}</div>
                  </a>
                ))}
              </div>
            </div>
